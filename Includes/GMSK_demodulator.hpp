...
public:
  ...
  std::pair<size_t, size_t> try_to_find_packet()
  {
    ...
    std::cout << "Maximal correlation with template preamble on position (from amplitude detected position) " << packet_precision_shift << std::endl;
    size_t packet_begin = ((packet_index.first + packet_precision_shift) > 0) ? packet_begin = packet_index.first + packet_precision_shift : packet_index.first;
    return {packet_index.first + packet_precision_shift, packet_index.second + packet_precision_shift};
  }
   ...
   */
  int get_shift_of_packet(std::vector<ext_sample> &realization, size_t packet_position, double normalization_coeff)
  {
    ...
    auto preamble_iterator = preamble_normed.begin(); /*Домножаем преамбулу на размерность принятой реализации*/
    auto correlation = get_correlation(preamble_iterator, packet_iterator, preamble_normed.size());
  ...
  std::vector<double> get_correlation(std::vector<std::complex<double>>::const_iterator x, std::vector<std::complex<double>>::const_iterator y, size_t n)
  {
    ...
    std::cout << "Size of vectors before FFT " << x_padded.size() << " " << y_padded.size() << std::endl;
   ...
   */
  inline std::pair<std::vector<ext_sample>, size_t> get_preamble(size_t samples_per_symbol) const
  {
  ...
  std::vector<bool> hard_decision(const std::vector<double> &metrics) const
  {
    ...
    for (size_t i = get_sample_per_symbol(); i < metrics.size() - get_sample_per_symbol(); i += get_sample_per_symbol())
    {
      double sum = 0;
